...
# Load in data
path_dict = paths()
mi_df = pd.read_csv(join(path_dict['save_path'], 'region_mi_150ms-bins.csv'))
...
for i, region_pair in enumerate(region_pairs):
    ...
    long_df = pd.melt(mi_df[mi_df['region_pair'] == region_pair],
                      id_vars='time', value_vars=['mi_goal_baseline', 'mi_distractor_baseline'])
    sns.lineplot(data=long_df, x='time', y='value', hue='variable', ax=axs[i], errorbar='se',
                 legend=None, hue_order=['mi_goal_baseline', 'mi_distractor_baseline'],
                 palette=[colors['goal'], colors['no-goal']])
    """
    this_df = mi_df[mi_df['region_pair'] == region_pair]
    axs[i].plot(this_df['time'], this_df['mi_goal_baseline'], color=colors['goal'])
    axs[i].fill_between(this_df['time'],
                        this_df['mi_goal_baseline'] - this_df['mi_sem_goal'],
                        this_df['mi_goal_baseline'] + this_df['mi_sem_goal'],
                        color=colors['goal'], alpha=0.25, lw=0)
    axs[i].plot(this_df['time'], this_df['mi_distractor_baseline'], color=colors['no-goal'])
    axs[i].fill_between(this_df['time'],
                        this_df['mi_distractor_baseline'] - this_df['mi_sem_distractor'],
                        this_df['mi_distractor_baseline'] + this_df['mi_sem_distractor'],
                        color=colors['no-goal'], alpha=0.25, lw=0)
    
    axs[i].set(title=f'{region_pair}')
    
    axs_sound[i].plot(this_df['time'], this_df['mi_sound_baseline'], color=colors['goal'])
    axs_sound[i].fill_between(this_df['time'],
                        this_df['mi_sound_baseline'] - this_df['mi_sem_sound'],
                        this_df['mi_sound_baseline'] + this_df['mi_sem_sound'],
                        color=colors['goal'], alpha=0.25, lw=0)
...
axs[0].plot(this_df['time'], this_df['mi_goal_baseline'], color=colors['goal'], label='Goal')
axs[0].fill_between(this_df['time'],
                    this_df['mi_goal_baseline'] - this_df['mi_sem_goal'],
                    this_df['mi_goal_baseline'] + this_df['mi_sem_goal'],
                    color=colors['goal'], alpha=0.25, lw=0)
axs[0].plot(this_df['time'], this_df['mi_distractor_baseline'], color=colors['no-goal'], label='No goal')
axs[0].fill_between(this_df['time'],
                    this_df['mi_distractor_baseline'] - this_df['mi_sem_distractor'],
                    this_df['mi_distractor_baseline'] + this_df['mi_sem_distractor'],
                    color=colors['no-goal'], alpha=0.25, lw=0)
...
axs[1].plot(this_df['time'], this_df['mi_goal_baseline'], color=colors['goal'])
axs[1].fill_between(this_df['time'],
                    this_df['mi_goal_baseline'] - this_df['mi_sem_goal'],
                    this_df['mi_goal_baseline'] + this_df['mi_sem_goal'],
                    color=colors['goal'], alpha=0.25, lw=0)
axs[1].plot(this_df['time'], this_df['mi_distractor_baseline'], color=colors['no-goal'])
axs[1].fill_between(this_df['time'],
                    this_df['mi_distractor_baseline'] - this_df['mi_sem_distractor'],
                    this_df['mi_distractor_baseline'] + this_df['mi_sem_distractor'],
                    color=colors['no-goal'], alpha=0.25, lw=0)
...
axs[2].plot(this_df['time'], this_df['mi_goal_baseline'], color=colors['goal'])
axs[2].fill_between(this_df['time'],
                    this_df['mi_goal_baseline'] - this_df['mi_sem_goal'],
                    this_df['mi_goal_baseline'] + this_df['mi_sem_goal'],
                    color=colors['goal'], alpha=0.25, lw=0)
axs[2].plot(this_df['time'], this_df['mi_distractor_baseline'], color=colors['no-goal'])
axs[2].fill_between(this_df['time'],
                    this_df['mi_distractor_baseline'] - this_df['mi_sem_distractor'],
                    this_df['mi_distractor_baseline'] + this_df['mi_sem_distractor'],
                    color=colors['no-goal'], alpha=0.25, lw=0)
...
sns.despine(trim=True)
plt.tight_layout()
plt.savefig(join(path_dict['google_drive_fig_path'], 'interarea_mi_goal_obj_entry.jpg'), dpi=600)
...
for i, region_pair in enumerate(plot_pairs):
    this_df = mi_df[mi_df['region_pair'] == region_pair]
    ax.plot(this_df['time'], this_df['mi_sound_baseline'], color=these_colors[i], label=region_pair)
...
sns.despine(trim=True)
plt.tight_layout()
plt.savefig(join(path_dict['google_drive_fig_path'], 'interarea_mi_sound_onset.jpg'), dpi=600)
