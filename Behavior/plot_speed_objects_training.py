...
for i, subject in enumerate(subjects['SubjectID']):
    ...
    for j, ses in enumerate(sessions):
        if isfile(join(path_dict['server_path'], 'Subjects', subject, ses, 'trials.csv')):
            ...
            wheel_speed = wheel_speed[::200] / 10
            wheel_times = wheel_times[::200]
            ...
            axs[j].set(ylabel='Speed (cm/s)', xticks=np.arange(-2, 7, 2), ylim=[0, 4],
                       title=f'{ses}', xlabel='')
            axs[j].plot([0, 0], axs[j].get_ylim(), color='grey', ls='--', zorder=0)
           
    f.suptitle(f'{subjects.iloc[i, 1]}')
    ...
    for j, ses in enumerate(sessions):
        if isfile(join(path_dict['server_path'], 'Subjects', subject, ses, 'trials.csv')):
            ...
            # Load in data
            reward_times = np.load(join(path_dict['server_path'], 'Subjects', subject, ses, 'reward.times.npy'))
            wheel_times = np.load(join(path_dict['server_path'], 'Subjects', subject, ses, 'wheel.times.npy'))
            wheel_speed = np.load(join(path_dict['server_path'], 'Subjects', subject, ses, 'wheel.speed.npy'))
