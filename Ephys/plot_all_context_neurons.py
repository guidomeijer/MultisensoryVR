...
for i, (subject, date, probe) in enumerate(zip(rec_df['subject'], rec_df['date'], rec_df['probe'])):
    ...
    print(f'Found {these_neurons.shape[0]} context neurons')
    ...
    for i, neuron_id in enumerate(these_neurons['neuron_id']):
        ...
        if not isdir(join(path_dict['fig_path'], 'ExampleNeurons', 'Context', f'{region}')):
            os.mkdir(join(path_dict['fig_path'], 'ExampleNeurons', 'Context', f'{region}'))
        ...
        peri_multiple_events_time_histogram(
            ...
            smoothing = T_SMOOTHING, ylim=y_max_use,
            pethline_kwargs=[{'color': colors['context1'], 'lw': 1}, {'color': colors['context2'], 'lw': 1}],
            errbar_kwargs=[{'color': colors['context1'], 'alpha': 0.3, 'lw': 0}, {'color': colors['context2'], 'alpha': 0.3, 'lw': 0}],
            raster_kwargs=[{'color': colors['context1'], 'lw': 0.5}, {'color': colors['context2'], 'lw': 0.5}],
            eventline_kwargs={'lw': 0}, include_raster=True)
        ...
        # Save
        plt.savefig(join(path_dict['fig_path'], 'ExampleNeurons', 'Context', f'{region}',
