...
for i, (subject, date, probe) in enumerate(zip(rec_df['subject'], rec_df['date'], rec_df['probe'])):
    ...
    for i, neuron_id in enumerate(these_neurons['neuron_id']):
        ...
        if not isdir(join(path_dict['fig_path'], 'SoundNeurons', f'{region}')):
            os.mkdir(join(path_dict['fig_path'], 'SoundNeurons', f'{region}'))
        
        # %% 
        if not isfile(join(path_dict['fig_path'], 'SoundNeurons', f'{region}',
            ...
            # Save
            plt.savefig(join(path_dict['fig_path'], 'SoundNeurons', f'{region}',
