...
for i, (subject, date) in enumerate(zip(rec['subject'], rec['date'])):
    ...
    for region, probe in zip(regions, region_probes):
        ...
        f, axs = plt.subplots(1, N_PATTERNS, figsize=(1.75*N_PATTERNS, 1.75), dpi=dpi)
        for pp in range(N_PATTERNS):
            peri_event_trace(amplitudes[pp, :], time_ax, all_obj_df.loc[all_obj_df['object'] == 2, 'times'],
                             ...
                             t_before=3, t_after=1, ax=axs[pp],
                             color_palette=[colors['no-goal'], colors['goal']])
            axs[pp].set(xticks=np.arange(-3, 1), yticks=[], ylabel='')
        plt.savefig(path_dict['google_drive_fig_path'] / 'SpikePatterns' / f'{region}_{subject}_{date}.jpg', dpi=600)
        plt.close(f)
