...
fit_bin = d_centers[np.argmin(np.abs(d_centers - LDA_PLACE))]
lda_intervals = np.vstack((trials['enterEnvPos'] + (fit_bin - (BIN_SIZE/2)),
                           trials['enterEnvPos'] + (fit_bin + (BIN_SIZE/2)))).T
...
fit_bin_control = d_centers[np.argmin(np.abs(d_centers - LDA_CONTROL))]
control_intervals = np.vstack((trials['enterEnvPos'] + (fit_bin_control - (BIN_SIZE/2)),
                               trials['enterEnvPos'] + (fit_bin_control + (BIN_SIZE/2)))).T
