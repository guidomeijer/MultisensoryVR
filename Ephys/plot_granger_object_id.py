...
# Load in data
path_dict = paths()
granger_df = pd.read_csv(join(path_dict['save_path'], 'granger_causality_object_id.csv'))

session_avg = granger_df.groupby(['region1', 'region2', 'date'])['f_stat'].mean().reset_index()
mean_causality = session_avg.groupby(['region1', 'region2'])['f_stat'].mean().reset_index()

# Create an empty list to store results
combined_pval_list = []

# Loop over all unique (region1, region2) combinations
grouped = granger_df.groupby(['region1', 'region2'])
...
plt.tight_layout()
plt.show()
plt.savefig(join(path_dict['google_drive_fig_path'], 'granger_causality_object_id.jpg'), dpi=600)