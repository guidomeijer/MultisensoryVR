...
VIDEO_FOLDER_NAME = "raw_video_data"
...
for subject_name in os.listdir(source_dir):
    ...
    for session_name in os.listdir(subject_path):
        ...
        for filename in os.listdir(video_source_path):
            ...
            # Check for file type and convert to mp4 if .h264
            if filename.lower().endswith(".h264"):
                print(f"   [!] WARNING: File '{filename}' has a .h264 extension, compressing video")
                subprocess.call(['ffmpeg', '-i', full_source_path, '-vcodec', 'libx265', '-crf', '20',
                                 '-n', (full_source_path[:-5] + '.mp4')])
                os.remove(full_source_path)
                full_source_path = full_source_path[:-5] + '.mp4'
