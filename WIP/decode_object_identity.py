...
# Only select objects that are not rewarded
all_obj_df = all_obj_df[all_obj_df['goal'] == 0]

# Now there are twice as many control objects (because they are never rewarded)
# subselect half of them randomly
drop_index = np.random.choice(all_obj_df[all_obj_df['object'] == 3].index,
                              int(np.sum(all_obj_df['object'] == 3) / 2), replace=False)
all_obj_df.drop(index=drop_index, inplace=True)
...
for i, bin_center in enumerate(t_centers):
    ...
    for r, region in enumerate(np.unique(clusters['region'])):
        ...
        results = Parallel(n_jobs=-1)(
            delayed(classify_subselection)(region_counts, N_NEURONS, all_obj_df['object'].values, clf, kfold_cv)
        ...
        decode_df = pd.concat((decode_df, pd.DataFrame(data={
            'time': bin_center, 'accuracy': accuracy_obj, 'region': region})))
        
    # Save to disk
    decode_df.to_csv(join(path_dict['save_path'], 'decode_object_identity.csv'), index=False)
