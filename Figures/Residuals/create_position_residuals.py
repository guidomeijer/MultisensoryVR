...
def process_recording(i, subject, date, probe, n_recs, path_dict, rel_bin_centers):
    ...
    for neuron_col in neuron_columns:
        ...
        try:
            results = glm_nb.fit()
            residual_arr.append(results.resid_response.values / exposure_vector)
            these_results = results.params
            these_results['pseudo_rsquared'] = results.pseudo_rsquared()
            glm_results.append(these_results)
...
if __name__ == '__main__':
    ...
    if OVERWRITE:
        ...
        with open(path_dict['google_drive_data_path'] / f'residuals_position_{MIN_SPEED}mms.pickle', 'rb') as handle:
