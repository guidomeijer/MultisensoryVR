...
merged_df['sig_reward'] = (merged_df['p_reward_obj1'] < 0.05) | (merged_df['p_reward_obj2'] < 0.05)
...
# Summary statistics per session and region
per_ses_df = merged_df.groupby(['region', 'neuron_type', 'ses_id']).sum(numeric_only=True)
per_ses_df['n_neurons'] = merged_df.groupby(['region', 'neuron_type', 'ses_id']).size()
...
for region in per_ses_df['region'].unique():
    region_df = per_ses_df[per_ses_df['region'] == region]
    region_merged = merged_df[merged_df['region'] == region]
    
    # Align PYR and INT sessions for paired t-test
    pyr_df = region_df[region_df['neuron_type'] == 'PYR']
    int_df = region_df[region_df['neuron_type'] == 'INT']
    paired_df = pd.merge(pyr_df, int_df, on='ses_id', suffixes=('_pyr', '_int'))
    ...
    # Object onset
    if len(paired_df) > 1:
        _, p = stats.ttest_rel(paired_df['perc_obj_onset_pyr'], paired_df['perc_obj_onset_int'])
    else:
        p = np.nan
    print(f'{region} - Object onset; p = {p:.3f}')
    sns.barplot(region_df.sort_values(by='neuron_type', ascending=False), x='neuron_type', y='perc_obj_onset',
                hue='neuron_type', errorbar='se', ax=axs[0], palette=[colors['PYR'], colors['INT']])
    if not np.isnan(p) and p < 0.05:
        axs[0].text(0.5, 43, '*', fontsize=12, ha='center')
    axs[0].set(xlabel='', ylim=[0, 50], title='Object onset', ylabel='', yticks=[0, 25, 50])
    
    # Reward
    if len(paired_df) > 1:
        _, p = stats.ttest_rel(paired_df['perc_reward_pyr'], paired_df['perc_reward_int'])
    else:
        p = np.nan
    print(f'{region} - Reward; p = {p:.3f}')
    sns.barplot(region_df.sort_values(by='neuron_type', ascending=False), x='neuron_type', y='perc_reward',
    ...
    # Object 1
    if len(paired_df) > 1:
        _, p = stats.ttest_rel(paired_df['perc_context_obj1_pyr'], paired_df['perc_context_obj1_int'])
    else:
        p = np.nan
    print(f'{region} - Object 1; p = {p:.3f}')
    sns.barplot(region_df.sort_values(by='neuron_type', ascending=False), x='neuron_type', y='perc_context_obj1',
                hue='neuron_type', errorbar='se', ax=axs[2], palette=[colors['PYR'], colors['INT']])
    if not np.isnan(p) and p < 0.05:
        axs[2].text(0.5, 13, '*', fontsize=12, ha='center')
    axs[2].set(ylabel='', xlabel='', title='Rew. object 1', ylim=[0, 15], yticks=[0, 5, 10, 15])
    
    # Object 2
    if len(paired_df) > 1:
        _, p = stats.ttest_rel(paired_df['perc_context_obj2_pyr'], paired_df['perc_context_obj2_int'])
    else:
        p = np.nan
    print(f'{region} - Object 2; p = {p:.3f}')
    sns.barplot(region_df.sort_values(by='neuron_type', ascending=False), x='neuron_type', y='perc_context_obj2',
                hue='neuron_type', errorbar='se', ax=axs[3], palette=[colors['PYR'], colors['INT']])
    if not np.isnan(p) and p < 0.05:
        axs[3].text(0.5, 13, '*', fontsize=12, ha='center')
    axs[3].set(ylabel='', xlabel='', title='Rew. object 2')
    
    f.text(0.04, 0.5, f'{region} - Significant neurons (%)', va='center', rotation='vertical')
    
    sns.despine(trim=False)
    plt.subplots_adjust(left=0.22, bottom=0.1, right=0.95, top=0.9, hspace=0.4, wspace=0.3)
    plt.savefig(path_dict['paper_fig_path'] / 'Interneurons' / f'perc_sig_neurons_{region}.pdf')
