# -*- coding: utf-8 -*-
"""
Created on Fri Aug 21 16:15:00 2026
...
for date in unique_dates:
    ...
    for region in session_regions:
        ...
        # Create arrays per context (trials x spatial bins x neurons)
...
save_fig_dir = path_dict['paper_fig_path'] / 'LDA'
...
if n_pairs_far > 0:
    ...
    axs[0].set_ylabel('Context LDA distance correlation', labelpad=0)
    f.supxlabel('Position (cm)', fontsize=7, y=0.04)
    f.suptitle('FAR', fontsize=8)
    sns.despine(trim=True)
    plt.tight_layout()
    plt.savefig(save_fig_dir / f'lda_context_corr_far_{USE_TYPE}.pdf')
    plt.savefig(save_fig_dir / f'lda_context_corr_far_{USE_TYPE}.jpg', dpi=600)
    plt.show()
...
if n_pairs_near > 0:
    ...
    sns.despine(trim=True)
    plt.tight_layout()
    plt.savefig(save_fig_dir / f'lda_context_corr_near_{USE_TYPE}.pdf')
    plt.savefig(save_fig_dir / f'lda_context_corr_near_{USE_TYPE}.jpg', dpi=600)
    plt.show()
