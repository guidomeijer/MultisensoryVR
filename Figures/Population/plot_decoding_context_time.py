...
#ax1.plot([0, 0], [0.3, 0.8], color='grey', ls='--')
...
ax1.set(yticks=[0.4, 0.5, 0.6, 0.7, 0.8], yticklabels=[40, 50, 60, 70, 80], ylim=[0.35, 0.8],
        xticks=[-0.5, 0, 0.5, 1], xticklabels=[-0.5, 0, 0.5, 1],
        ylabel='Context decoding accuracy (%)', xlim=[-0.5, 1],
        xlabel='Time from object entry (s)')
ax1.legend(loc='upper left', bbox_to_anchor=(1.05, 1))
sns.despine(trim=True)
plt.tight_layout()
plt.savefig(path_dict['paper_fig_path'] / 'Decoding' / 'context_time.jpg', dpi=600)
plt.savefig(path_dict['paper_fig_path'] / 'Decoding' / 'context_time.pdf')
plt.show()