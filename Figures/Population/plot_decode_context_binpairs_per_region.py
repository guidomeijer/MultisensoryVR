...
for i, region in enumerate(PLOT_REGIONS):
    ...
    sns.heatmap(pivot_df, cmap='coolwarm', ax=axs[i], vmin=0.3, vmax=0.7, cbar=False)
    axs[i].plot([40, 40], [0, 150], color='w', ls='--', lw=0.5)
    axs[i].plot([0, 150], [40, 40], color='w', ls='--', lw=0.5)
    axs[i].plot([0, 150], [97.5, 97.5], color='w', ls='--', lw=0.5)
    axs[i].plot([97.5, 97.5], [0, 150], color='w', ls='--', lw=0.5)
    axs[i].plot([130, 130], [0, 150], color='w', ls='--', lw=0.5)
    axs[i].plot([0, 150], [130, 130], color='w', ls='--', lw=0.5)
...
# Add a single colorbar for all heatmaps to keep subplot widths equal
cbar = f.colorbar(axs[5].collections[0], ax=axs, fraction=0.03, pad=0.02)
...
for i, region in enumerate(PLOT_REGIONS):
    ...
    # Do statistics
    p_values[region] = np.full(np.unique(plot_df['train_position_cm']).shape[0], np.nan)
    for k, pos in enumerate(np.unique(plot_df['train_position_cm'])):
        p_values[region][k] = stats.ttest_1samp(plot_df.loc[plot_df['train_position_cm'] == pos, 'accuracy'], 0.5)[1]
    
    sns.lineplot(data=plot_df, x='train_position_cm', y='accuracy', errorbar='se', ax=axs[i],
                 err_kws={'lw': 0}, color='k')
    add_significance(np.unique(plot_df['train_position_cm']), p_values[region], ax=axs[i], y_pos=0.78)
