...
ax1.plot([0, 0], [0, 1.3], lw=0.5, ls='--', color='grey')
...
ax1.set(xticks=np.arange(-2, 1.5), xlabel='', yticks=[0, 0.3, 0.6, 0.9, 1.2], ylim=[0, 1.3],
        yticklabels=[0, 0.3, 0.6, 0.9, 1.2], title='Rewarded object 1', ylabel='Assembly activation')

ax2.plot([0, 0], [0, 1.3], lw=0.5, ls='--', color='grey', zorder=0)
peri_event_trace(activations_smooth[plot_assembly, :], binned_time,
                 all_obj_df.loc[all_obj_df['object'] == 2, 'times'],
                 all_obj_df.loc[all_obj_df['object'] == 2, 'goal'].values + 1,
                 t_before=2, t_after=1, ax=ax2,
                 color_palette=[colors['no-goal'], colors['goal']])
ax2.text(-0.5, 1.2, '**', fontsize=11, ha='center', va='center')
