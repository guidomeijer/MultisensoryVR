...
for i, (subject, date, probe) in enumerate(zip(rec['subject'], rec['date'], rec['probe'])):
    ...
    amp_paths = (path_dict['google_drive_data_path'] / 'Assemblies').glob(f'{subject}_{date}_{probe}*.amplitudes.npy')
    ...
    for amp_path in amp_paths:
        ...
        # Plot
        n_cols = np.ceil(np.sqrt(amplitudes.shape[0])).astype(int)
        n_rows = np.ceil(amplitudes.shape[0] / n_cols).astype(int)
        f, axs = plt.subplots(n_rows, n_cols, figsize=(7, 4), dpi=dpi, squeeze=False)
        axs = axs.flatten()
        for pat in range(amplitudes.shape[0]):
        ...
        f.text(0.04, 0.5, 'Assembly activation (a.u.)', ha='center', va='center', rotation='vertical')
        f.suptitle(region)
        sns.despine(trim=True)
        plt.subplots_adjust(left=0.11, bottom=0.12, right=0.98, top=0.95, wspace=0.15, hspace=0.2)

        plt.savefig(
            path_dict['google_drive_fig_path'] / 'Assemblies' / 'Timewarped' / f'{region}_{subject}_{date}_{probe}.jpg',
            dpi=600)
        plt.close(f)
