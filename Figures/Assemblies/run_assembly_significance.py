...
for i, (subject, date) in enumerate(zip(rec['subject'], rec['date'])):
    ...
    for j, this_region in enumerate(list(amplitudes.keys())):
        ...
        if PLOT & (n_assemblies > 1):
            f, axs = plt.subplots(2, n_assemblies, figsize=(1.75 * n_assemblies, 3.5),
                                  sharex=True, dpi=dpi)
            ...
            plt.savefig(
                path_dict['google_drive_fig_path'] / 'Assemblies' / f'{this_region}_{subject}_{date}_reward.jpg',
                dpi=600)
            plt.close(f)
        ...
        if PLOT & (n_assemblies > 1):
            # Plot
            f, axs = plt.subplots(1, n_assemblies, figsize=(1.75 * n_assemblies, 1.75), dpi=dpi)
            ...
            plt.tight_layout()
            plt.savefig(
                path_dict['google_drive_fig_path'] / 'Assemblies' / f'{this_region}_{subject}_{date}_sound.jpg',
                dpi=600)
            plt.close(f)
        ...
        if these_ripples.shape[0] >= MIN_RIPPLES:
            ...
            if PLOT & (n_assemblies > 1):
                # Plot
                f, axs = plt.subplots(1, activation_rate.shape[0], figsize=(1.75 * activation_rate.shape[0], 1.75),
                                      dpi=dpi)
                for pp in range(activation_rate.shape[0]):
