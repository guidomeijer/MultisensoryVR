...
for i, this_ses in enumerate(np.unique(spike_dict['date'])):
    ...
    for j in np.where(np.array(spike_dict['date']) == this_ses)[0]:
        ...
        for r, region in enumerate(unique_regions):
            ...
            pop_vec['First   B'] = np.mean(spike_counts[(spatial_bins == FIRST_OBJ + AFTER_OBJ) & (context_per_bin == 3 - obj1_goal), :], axis=0)
            if is_far:
                pop_vec['Between   A'] = np.mean(spike_counts[(spatial_bins == BRIDGE_FAR) & (context_per_bin == obj1_goal), :], axis=0)
                pop_vec['Between   B'] = np.mean(spike_counts[(spatial_bins == BRIDGE_FAR) & (context_per_bin == 3 - obj1_goal), :], axis=0)
                pop_vec['Second   A'] = np.mean(spike_counts[(spatial_bins == FAR_OBJ + AFTER_OBJ) & (context_per_bin == obj2_goal), :], axis=0)
                pop_vec['Second   B'] = np.mean(spike_counts[(spatial_bins == FAR_OBJ + AFTER_OBJ) & (context_per_bin == 3 - obj2_goal), :], axis=0)
            else:
                pop_vec['Between   A'] = np.mean(spike_counts[(spatial_bins == BRIDGE_NEAR) & (context_per_bin == obj1_goal), :], axis=0)
